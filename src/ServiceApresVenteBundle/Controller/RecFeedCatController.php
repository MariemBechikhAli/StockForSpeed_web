    ...
    public function createCategorieAction(Request $request)
    {
       ...
       // $form = $this->createForm(RecFeedCatType::class, $recfeedcat);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $uploadedFile->move($this->getParameter('kernel.project_dir').'/web/uploads/catRecFeed_image',$filename);

            $entity->setImage($filename);
          //  $recfeedcat->setDatefeedback(new \DateTime('now'));
    ...
    }  private function createEditForm(RecFeedCat $entity)
{
    ...
    public function updateCategorieAction(Request $request ,$id)
    {
        ...
        if ($editForm->isValid()) {
            ...
            $file->move($this->getParameter('kernel.project_dir').'/web/uploads/catRecFeed_image',$filename);
            $entity->setImage($filename);
            $em=$this->getDoctrine()->getManager();
            $em->flush();
